...
# from PIL import ImageTk, Image  # Uncomment if you have tap.png
...
class Controller:
    ...
    def get_position(hand_result):
        ...
        x = int(position[0] * sx)  # Fixed: No inversion for correct left-right
        y = int(position[1] * sy)
        ...
        if distsq <= 25:
            ...
            ratio = 2.1
        x, y = Controller.prev_hand[0] + delta_x*ratio, Controller.prev_hand[1] + delta_y*ratio
        return (x, y)
...
class GestureController:
    ...
    def __init__(self):
        ...
        GestureController.cap = cv2.VideoCapture(1)  # USB webcam
        if not GestureController.cap.isOpened():
            print("Error: Could not open webcam. Try index 0, 1, 2, etc.")
            exit()
    ...
    def start(self):
        ...
        with mp_hands.Hands(max_num_hands=2, min_detection_confidence=0.6, min_tracking_confidence=0.6, model_complexity=0) as hands:
            while GestureController.cap.isOpened() and GestureController.gc_mode:
                ...
                # Optional: Further reduce resolution
                # image = cv2.resize(image, (320, 240))
                frame_count += 1
                if frame_count % 2 == 0:
                    threading.Thread(target=process_hands, args=(image,), daemon=True).start()
                if not results_queue.empty():
                    ...
                    if results.multi_hand_landmarks:
                        ...
                        # Optional: Draw landmarks (uncomment for visualization)
                        # for hand_landmarks in results.multi_hand_landmarks:
                        #     mp_drawing.draw_landmarks(image, hand_landmarks, mp_hands.HAND_CONNECTIONS)
                    else:
                        Controller.prev_hand = None
                # Optional: Display frame (uncomment for visualization)
...
if __name__ == "__main__":
    ...
    # Uncomment the following if you have tap.png
